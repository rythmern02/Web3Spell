...
const Navbar = () => {
  ...
  const toggleMenu = () => {
    setIsOpen(!isOpen);
    console.log("hello I am Rythme the value is ", isOpen);
  };
      ...
      return (
        <div
          className={`navbar fixed top-0 right-0 w-full z-50 transition-transform duration-500 ease-in-out ${
            isOpen ? "translate-y-0 " : "-translate-y-full mt-20"
          }`}
        >
          <button
            onClick={toggleMenu}
            ...
            <div className="w-full h-[65px] fixed top-0 mt-0 shadow-lg shadow-[#2A0E61]/50 bg-[#03001417] backdrop-blur-md z-50 px-10">
              <div className="w-full h-full flex flex-col items-center  m-auto px-[10px]">
                <div className="w-[500px] h-full flex flex-row items-center justify-between md:mr-20">
                  <div className="mt-auto flex flex-col items-center justify-between w-full h-auto border border-[#7042f861] bg-[#0300145e] mr-[15px] px-[20px] py-[10px] rounded-full text-gray-200">
              <button
                onClick={toggleMenu}
                className={`hamburger-button focus:outline-none justify-self-end text-white font-bold py-2 px-4 rounded`}
              >
                <Image src={"/close.png"} className="opacity-70" width={30} height={30} alt="" />
              </button>
